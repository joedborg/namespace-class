...
```sh
make docker-build docker-push IMG=<some-registry>/namespace-class:tag
```
...
```sh
make deploy IMG=<some-registry>/namespace-class:tag
```
...
```sh
make build-installer IMG=<some-registry>/namespace-class:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/namespace-class/<tag or branch>/dist/install.yaml
```
